...
///
/// Allows users to select 4 player cards + 1 synergy card for defense
/// Uses defense-available-cards API (cards not in attack lineup)
class DefenseLineupSelectionScreen extends StatefulWidget {
  const DefenseLineupSelectionScreen({super.key});

  @override
  State<DefenseLineupSelectionScreen> createState() => _DefenseLineupSelectionScreenState();
}

class _DefenseLineupSelectionScreenState extends State<DefenseLineupSelectionScreen> {
  ...
  Future<void> _saveLineup() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Defense lineup saved successfully!'),
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text(
              'No cards available for defense',
              style: TextStyle(
                color: Colors.grey,
                fontSize: 16,
              ),
