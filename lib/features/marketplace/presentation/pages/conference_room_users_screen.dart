...
class _ConferenceRoomUsersScreenState extends State<ConferenceRoomUsersScreen> {
  ...
  Future<void> _checkActiveAttackAndLoad() async {
    ...
    if (activeMatch != null) {
      // Verify with server if this match is still valid/active
      // This prevents the user from being stuck if local storage is stale
      try {
        ...
        if (serverMatch == null) {
          // Match not found on server - clear local and proceed
        ...
        if (serverMatch.status != 'PREPARATION' && serverMatch.status != 'IN_PROGRESS') {
          // Match is finished on server - clear local and proceed
          await MatchStorageService.clearMatch();
          if (mounted) _loadUsers();
          return;
        }

        // Match is confirmed active on server
        if (mounted) {
          _showAlreadyAttackingDialog(activeMatch);
        }
      } catch (e) {
        // If network fail, fallback to local blocking but allow retry
        if (mounted) {
          _showAlreadyAttackingDialog(activeMatch);
        }
      }
