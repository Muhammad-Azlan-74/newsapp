...
/// Defense Lineup View Screen
///
/// Shows defense lineup with:
...
class DefenseLineupViewScreen extends StatefulWidget {
  const DefenseLineupViewScreen({super.key});

  @override
  State<DefenseLineupViewScreen> createState() => _DefenseLineupViewScreenState();
}

class _DefenseLineupViewScreenState extends State<DefenseLineupViewScreen> {
  ...
  // Defense status
  DefenseMatchData? _defenseMatch;
  Timer? _statusTimer;
  ...
  Future<void> _loadDefenseStatus() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _defenseMatch = response.data;
        });

        // Save pending result for defense if under attack
        if (response.data != null && response.data!.preparationDeadline != null) {
          await MatchResultService.savePendingResult(
            matchId: response.data!.id,
            opponentName: response.data!.attacker.fullName,
            isAttacker: false,
            matchEndTime: response.data!.preparationDeadline!,
          );
        }
      }
    } catch (e) {
      // Silently fail - just show "not under attack"
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      // Load available cards and existing defense lineup in parallel
      final results = await Future.wait([
        _cardRepository.getDefenseAvailableCards(),
        _cardRepository.getDefenseLineup(),
      ]);

      final cards = results[0] as List<UserCard>;
      final defenseLineup = results[1] as AttackLineupResponse;
      ...
      setState(() {
        ...
        // Pre-populate with existing defense lineup if any
        if (defenseLineup.data != null) {
          int slotIndex = 0;
          for (final savedCard in defenseLineup.data!.playerCards) {
          ...
          if (defenseLineup.data!.synergyCard != null) {
            _selectedSlots[4] = cards.cast<UserCard?>().firstWhere(
              (c) => c!.id == defenseLineup.data!.synergyCard!.id,
              orElse: () => null,
            );
          }
        }
    ...
    if (_hasChanges && !_isLineupComplete) {
      final shouldLeave = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            'Your defense lineup is incomplete (${_selectedPlayerCount}/4 players, ${_selectedSynergyCard != null ? "1" : "0"}/1 synergy). Changes will not be saved.',
  ...
  Future<void> _saveLineup({bool showMessage = false}) async {
    ...
    try {
      ...
      if (showMessage) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Defense lineup saved!'),
    ...
    return PopScope(
      ...
      child: Stack(
        children: [
          ...
          // Dark overlay with blue tint for defense
          Positioned.fill(
            child: Container(
  ...
  Widget _buildStatusStrip() {
    final isUnderAttack = _defenseMatch != null;
    final remaining = isUnderAttack && _defenseMatch!.preparationDeadline != null
        ? _defenseMatch!.preparationDeadline!.difference(DateTime.now())
        : Duration.zero;
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            isUnderAttack ? Icons.warning_amber : Icons.shield,
            color: isUnderAttack ? Colors.orange : Colors.green,
            size: 20,
          ),
          ...
          Expanded(
            child: Text(
              isUnderAttack
                  ? 'Under attack by ${_defenseMatch!.attacker.fullName}'
                  : 'Not under attack',
              style: TextStyle(
                color: isUnderAttack ? Colors.orange[100] : Colors.green[100],
                fontSize: 13,
                fontWeight: FontWeight.w600,
              ),
            ),
          ),
          if (isUnderAttack && !remaining.isNegative)
            Container(
        ...
        children: alignRight
            ? [
                Text(
                  '$value',
                  style: TextStyle(
                    color: value > 0 ? Colors.cyanAccent : Colors.white54,
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
                  ),
                ),
      ...
      child: Container(
        ...
        child: card != null
            ...
            : Column(
                ...
                children: [
                  Icon(
                    isSynergySlot ? Icons.auto_awesome : Icons.shield,
                    color: isSynergySlot
                        ? Colors.purple.withOpacity(0.5)
